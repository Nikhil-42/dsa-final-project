...
class Maze(ShowBase):
    ...
    def setupCameraControls(self, props: WindowProperties):
        ...
        def move_camera(direction):
            self.camera.setPos(self.camera.getPos() + render.getRelativeVector(
                camera, tuple(np.array(direction) * SPEED * globalClock.getDt())))
        ...
        def rotate_camera(task):
            if self.mouseWatcherNode.hasMouse():
                ...
                self.win.movePointer(0, props.getXSize() //
                                     2, props.getYSize() // 2)
            return task.cont
    ...
    def setupRunners(self, rotation):
        # Display the video
        self.video.setTime(max(
            (self.run_data[agent]['finish_timestamps'][rotation-1] for agent in self.run_data)))
        ...
        for agent in self.run_data:
            ...
            self.taskMgr.doMethodLater(self.run_data[agent]['finish_timestamps'][rotation], self.follow_path,
                                       "FollowPath"+agent, extraArgs=[self.run_data[agent]["paths"][rotation], agent])

        self.taskMgr.doMethodLater(max((self.run_data[agent]['finish_timestamps'][rotation]
                                   for agent in self.run_data)), self.recurseSetup, "RecurseSetup", extraArgs=[rotation])
    ...
    def __init__(self):
        ...
        self.maze_gray = cv2.imread(
            "generated/maze_gray.png", cv2.IMREAD_GRAYSCALE)
        self.video = self.loadTexture(
            "generated/maze.mpg")  # imports video onto map
        self.video.stop()

        self.rotation_frames = []
        for rotation in range(4):
            self.rotation_frames.append(self.loadTexture(
                f"generated/solved_maze_{rotation}.png"))
        ...
        dy = 0.1
        for agent in self.run_data:
            self.addText(f"{agent}: {round((sum(self.run_data[agent]['times']) * 1000), 2)} ms.", (-1.7, 0, y), tuple(
                self.run_data[agent]["color"][::-1] + [1]))
            self.addText(f"{(sum(self.run_data[agent]['path_lengths'])) } distance", (1.2, 0, y), tuple(
                self.run_data[agent]["color"][::-1] + [1]))
        ...
        self.camera.setPos((self.maze.shape[1] * self.maze_node.getSx(
        ) / 2, self.maze.shape[0] * self.maze_node.getSy() / 2, 500))
